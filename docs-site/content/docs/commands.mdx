...
|---|---|
| [`data-contract`](/docs/commands/data-contract) | Declare sources, schemas, PII strategy, bias audit, drift thresholds |
| [`threat-model`](/docs/commands/threat-model) | Generate a STRIDE threat model for the active spec |
| [`model-governance`](/docs/commands/model-governance) | Pin model version, define eval baselines, write rollback plan |
| [`guardrails`](/docs/commands/guardrails) | Generate versioned system prompt + guardrail YAML for AI features |
| [`gate-check`](/docs/commands/gate-check) | Run all six gates against the active spec, emit verdict |
| [`audit`](/docs/commands/audit) | Post-implementation scan — inline prompts, secrets, direct SDKs |
| [`red-team`](/docs/commands/red-team) | Generate adversarial scenarios; `--run` hits staging automatically |
| [`install-rules`](/docs/commands/install-rules) | Install dev rules into docs, constitution, and agent context file |
...
Four of the eight commands are wired to Spec Kit phase hooks and fire
automatically. The other four are manual — you invoke them when you
need them.
...
- `data-contract`, `threat-model`, `red-team` — can also be run manually
  if you opt out of the prompts
